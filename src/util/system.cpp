...
const char * const BITCOIN_CONF_FILENAME = "bharatcoin.conf";
...
#else
    const char* pszModule = "bharatcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Bharatcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Bharatcoin";
#else
    // Unix
    return pathRet / ".bharatcoin";
#endif
#endif
}
