Bharatcoin Core version 0.18.1 is now available from:

  <https://download.bharatcoin.org/bharatcoin-0.18.1/>
...
To receive security and update notifications, please subscribe to:
  ...
  <https://groups.google.com/forum/#!forum/bharatcoin-dev>
...
`/Applications/Bharatcoin-Qt` (on Mac) or `bharatcoind`/`bharatcoin-qt` (on
Linux).
...
Bharatcoin Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Bharatcoin Core on unsupported systems.

Bharatcoin Core should also work on most other Unix-like systems but is not
as frequently tested on them.

From 0.17.0 onwards, macOS <10.10 is no longer supported. 0.17.0 is
built using Qt 5.9.x, which doesn't support versions of macOS older than
10.10. Additionally, Bharatcoin Core does not yet change appearance when
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/bharatcoin-project/bharatcoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@bharatcoin.org) the Bharatcoin Core
  project to let us know about their use-case, as this feature could be
  deprecated in the future).

Wallet changes
--------------
When creating a transaction with a fee above `-maxtxfee` (default 0.1 BHT),
the RPC commands `walletcreatefundedpsbt` and  `fundrawtransaction` will now fail
instead of rounding down the fee. Beware that the `feeRate` argument is specified
in BHT per kilobyte, not litoshi per byte.

Documentation
-------------

- A new short [document](https://github.com/bharatcoin-project/bharatcoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/bharatcoin-project/bharatcoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/bharatcoin-project/bharatcoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/bharatcoin-project/bharatcoin/blob/master/doc/bharatcoin-conf.md)
  about the `bharatcoin.conf` file describes how to use it to configure
  Bharatcoin Core.

- A new document introduces Bharatcoin Core's BIP174 [Partially-Signed
  Bharatcoin Transactions
  (PSBT)](https://github.com/bharatcoin-project/bharatcoin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/bharatcoin-project/bharatcoin/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Bharatcoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Bharatcoin Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/bharatcoin-project/bharatcoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/bharatcoin-project/bharatcoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Bharatcoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Bharatcoin Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/bharatcoin-project/bharatcoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/bharatcoin-project/bharatcoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart bharatcoind with the `-deprecatedrpc=generate`
...
- A new `bharatcoin-wallet` tool is now distributed alongside Bharatcoin
...
This section describes planned changes to Bharatcoin Core that may affect
other Bharatcoin software and services.

- Since version 0.16.0, Bharatcoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Bharatcoin Core 0.20 (expected about
  a year after 0.18), Bharatcoin Core will default to native segwit
  ...
  to bech32 addresses, and if the Bharatcoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Bharatcoin Core 0.20.
  ...
  set the `addresstype=bech32` configuration option in any Bharatcoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Bharatcoin Core
...
- A sub-project of Bharatcoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Bharatcoin Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Bharatcoin Core's own implementation, although entropy
  gathered by Bharatcoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Bharatcoin Core a
...
- On macOS, Bharatcoin Core now opts out of application CPU throttling
